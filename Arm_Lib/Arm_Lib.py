...
class Arm_Device(object):
    ...
    def Arm_serial_servo_write6_array(self, joints, time):
        ...
        if s1 > 270 or s2 > 270 or s3 > 270 or s4 > 270 or s5 > 270 or s6 > 270:
            print("参数传入范围不在0-180之内！")
            return
        try:
            pos = int((3100 - 900) * (s1 - 0) / (270 - 0) + 900)
            value1_H = (pos >> 8) & 0xFF
            value1_L = pos & 0xFF

            s2 = 180 - s2
            pos = int((3100 - 900) * (s2 - 0) / (270 - 0) + 900)
            value2_H = (pos >> 8) & 0xFF
            value2_L = pos & 0xFF

            s3 = 180 - s3
            pos = int((3100 - 900) * (s3 - 0) / (270 - 0) + 900)
            value3_H = (pos >> 8) & 0xFF
            value3_L = pos & 0xFF

            s4 = 180 - s4
            pos = int((3100 - 900) * (s4 - 0) / (270 - 0) + 900)
            value4_H = (pos >> 8) & 0xFF
            value4_L = pos & 0xFF
            ...
            pos = int((3100 - 900) * (s6 - 0) / (270 - 0) + 900)
            value6_H = (pos >> 8) & 0xFF
            value6_L = pos & 0xFF
            time_H = (time >> 8) & 0xFF
            time_L = time & 0xFF
    ...
    def Arm_serial_servo_read(self, id):
        ...
        if id == 5:
            ...
            pos = int((270 - 0) * (pos - 900) / (3100 - 900) + 0)
            if pos > 270 or pos < 0:
                return None
